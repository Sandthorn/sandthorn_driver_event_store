    ...
    describe "benchmark", benchmark: true do
      ...
      describe "save" do
        it "one event save 500 times" do
          Benchmark.bm do |x|
            x.report("new change save find") { for i in 1..n; es.save_events(test_events_one_event, 0, i.to_s, SandthornDriverEventStore::EventStore); end }
          end
        end
        it "two events save 500 times" do
          Benchmark.bm do |x|
            x.report("new change save find") { for i in 1..n; es.save_events(test_events_two_events, 0, i.to_s, SandthornDriverEventStore::EventStore); end }
          end
        end
        it "twenty events save 500 times" do
          Benchmark.bm do |x|
            x.report("new change save find") { for i in 1..n; es.save_events(test_events_20_events, 0, i.to_s, SandthornDriverEventStore::EventStore); end }
          end
        end
      end
      ...
      describe "find" do
        it "should find one event 500 times" do
          es.save_events(test_events_one_event, 0, aggregate_id, SandthornDriverEventStore::EventStore)

          Benchmark.bm do |x|
            x.report("find") { for i in 1..n; es.get_aggregate(aggregate_id, SandthornDriverEventStore::EventStore);  end }
          end
        end
        it "should find two events 500 times" do
          es.save_events(test_events_two_events, 0, aggregate_id, SandthornDriverEventStore::EventStore)
        ...
        it "should find twenty events 500 times" do
          es.save_events(test_events_20_events, 0, aggregate_id, SandthornDriverEventStore::EventStore)
       ...
       describe "find with snapshot" do
        ...
        it "should find one event that is snapshoted 500 times" do
          ...
          es.save_events(test_events_one_event, 0, aggregate_id, SandthornDriverEventStore::EventStore)
          es.save_snapshot(snapshot_data, aggregate_id, SandthornDriverEventStore::EventStore)
      ...
      #   Benchmark.bm do |x|
      #     x.report("new change save find") { for i in 1..n; es.save_events(test_events_a, 0, i.to_s, SandthornDriverEventStore::EventStore); es.get_aggregate(i.to_s, SandthornDriverEventStore::EventStore);  end }
      #   end
